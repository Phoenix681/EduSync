...
const CreateModule = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // NEW: Automatically attach the correct order number to each slide
      const formattedSlides = slides.map((slide, index) => ({
        ...slide,
        order: index + 1 // Slide 1 gets order 1, Slide 2 gets order 2, etc.
      }));

      // Swap out `slides` for `formattedSlides` here
  ...
  // Security Check: Only allow Educators
